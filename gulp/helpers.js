...
module.exports = {
    ...
    outputChannel: function (path, outputFile, type) {
        if (!allAssets) {
            if (args.sass && ['styles', 'styles-by-demo', 'components_styles'].indexOf(type) === -1) {
                return lazypipe().pipe(function () {
                    // noop
                });
            }
            if (args.js && ['scripts', 'components_scripts'].indexOf(type) === -1) {
                return lazypipe().pipe(function () {
                    // noop
                });
            }
                ...
                if (bundle.output.hasOwnProperty(type)) {
                    switch (type) {
                        ...
                        case 'components_styles':
                            ...
                            break;
                        case 'scripts':
                       	case 'components_scripts':
                            stream = gulp.src(bundle.src[type], {allowEmpty: true}).pipe(include({
                            	includePaths: [
