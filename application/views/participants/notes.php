...
if ($notes->num_rows() == 0) {
	...
	<div class="alert alert-info">
		No notes found. Do you want to <?php echo anchor('participants/notes/'.$familyID.'/new/', 'create one'); ?>?
	</div>
	<?php
} else {
	?>
				...
				<tbody>
					...
					foreach ($notes->result() as $row) {
						...
						<tr>
							...
							<td class="name">
								<?php echo anchor('participants/notes/edit/' . $row->noteID, $row->summary); ?>
							</td>
							<td>
								<div class='text-right'>
									<a class='btn btn-warning btn-sm' href='<?php echo site_url('participants/notes/edit/' . $row->noteID); ?>' title="Edit">
										<i class='far fa-pencil'></i>
									</a>
									<a class='btn btn-danger btn-sm confirm-delete' href='<?php echo site_url('participants/notes/remove/' . $row->noteID); ?>' title="Remove">
