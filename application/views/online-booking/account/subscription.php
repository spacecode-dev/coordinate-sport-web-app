    ...
    <div class="col-sm-12">
        <div class="box bordered-box">
            <div class="box-content box-double-padding">
                <h2 class="h3 light"><?php echo $subdata->subName ?></h2>
                <p class="event-specs">
                    <span><strong>Frequency:</strong> <?php echo ucfirst($subdata->frequency) ?></span><br />
                    <span><strong>Price:</strong> <?php echo currency_symbol($this->cart_library->accountID) . ucfirst($subdata->price) ?></span><br />
                    <span><strong>Number of Sessions per Week:</strong> <?php echo $subdata->no_of_sessions_per_week ?></span> <br />
                    <span><strong>Status:</strong> <?php
						if(count($subscription_status) > 0){
							foreach($subscription_status as $praticipants){
								foreach($praticipants as $value) {
									echo '<span class="badge badge-primary">'.ucfirst($value['status']).'</span>';
									break 2;
								}
							}
						}else{
                ...
                <p class="event-specs">
					...
					<fieldset class="card card-custom"  style="display:none">
						...
						<fieldset class="card card-custom">
							<div class="card-body">
								...
								if(count($subs) > 0):
									?>
									<div id="subscriptions_section" style="display:none">
															...
															<?php
																...
																if (intval($block->require_all_sessions) === 1 || ($block->autodiscount === 'fixed' && $block->autodiscount_amount > 0)) {
																	?> <span class="block_totals" >
																		<span class="price" style="display:none"><?php echo currency_symbol($this->cart_library->accountID); ?><span>0</span></span>
						...
						<?php
						if(count($subscription_status) == 0){
							?><a id="cancel-subscription" href="javascript:void(0);" data-redirect="<?php echo site_url('account/cancel_subscription/' . $subdata->psID); ?>" class="btn mr-3">Cancel</a> &nbsp;&nbsp;<?
						}
						?>
						<button class="btn" >Update</button>
						<div id="add-to-cart" class="d-none"></div>
					<?php
					echo form_close();
					?>
				</p>
		...
		<div class="modal-content">
			<div class="modal-body">
				<div style="text-align:center">
					<i class='far fa-exclamation-circle' style="font-size: 3rem"></i>
				</div>
				<br/>
				<div style="text-align:center">
					<p>Are you sure you want to cancel this subscription?</p><br />
				</div>
				<div style="text-align:center">
