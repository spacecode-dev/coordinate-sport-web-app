		...
		echo form_fieldset('', ['class' => 'card card-custom']);	?>
			<div class='card-header'>
				<div class="card-title">
					...
					<h3 class="card-label">Applies To</h3>
				</div>
			</div>
			<div class="card-body">
				<div class='multi-columns'>
					<div class="form-group">
						...
						foreach ($lesson_list as $lessonID => $lesson) {
							...
							if ($this->input->post()) {
								$lesson_list_array = $this->input->post('lesson_list');
							}
							if (!is_array($lesson_list_array)) {
								$lesson_list_array = array();
							}
							if (in_array($lessonID, $lesson_list_array)) {
								$data['checked'] = TRUE;
							}
							if ($prev_block != $lesson['block']) {
								?><p><?php echo $lesson['block']; ?>:</p><?php
								$prev_block = $lesson['block'];
							}
							?><div class="checkbox-single">
								<label class="checkbox">
									<?php echo form_checkbox($data); ?>
