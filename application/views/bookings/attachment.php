		...
		echo form_fieldset('', ['class' => 'card card-custom']);
			?><div class='card-header'>
				<div class="card-title">
					...
					<h3 class="card-label">Blocks <small>Attachment will only be sent with confirmations for the selected block(s)</small></h3>
				</div>
			</div>
			<div class="card-body">
				...
				<div class='multi-columns'>
					<div class="form-group">
						<?php
						foreach ($block_list as $blockID => $block) {
							$data = array(
								'name' => 'block_list[]',
								'value' => $blockID
							);
							...
							if ($this->input->post()) {
								$block_list_array = $this->input->post('block_list');
							}
							if (!is_array($block_list_array)) {
								$block_list_array = array();
							}
							if (in_array($blockID, $block_list_array)) {
								$data['checked'] = TRUE;
							}
							?><div class="checkbox-single">
								<label class="checkbox">
