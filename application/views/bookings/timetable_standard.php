...
<div class="row" id="timetable_desktop">
				...
				<table class='table table-striped table-bordered timetable'>
					...
					foreach ($time_slots as $slot => $label) {
						...
						foreach ($days as $day) {
							...
							$background_color = '';
							$tooltip = array();
							if(($search_fields["staff_id"] != null && $search_fields["staff_id"] != "") || $only_own === TRUE){
								$i++;
	...
	<div class='col-sm-2 timetable-legend'>
		<?php echo form_open($timetable_base . $search_extra); ?>
			<h3>Jump to Week</h3>
			<p><?php
			$options = array();
		...
		echo form_close();
		?>
		<h3><?php echo $this->settings_library->get_label('brands'); ?></h3>
		<div>
			<?php
			if ($brands->num_rows() > 0) {
				foreach ($brands->result() as $row) {
					?><span class="label label-inline label-brand-<?php echo $row->brandID; ?>"
							style="<?php echo label_style($row->colour); ?>"
							data-toggle="label-brand-<?php echo $row->brandID; ?>"><?php echo $row->name; ?></span><?php
				}
			}
			?>
			<span class="label label-inline label-pink label-nostaff"
				  data-toggle="label-nostaff">No/Missing Staff</span>
		</div>
		<h3>Activities</h3>
		<div>
			<?php
			if ($activities->num_rows() > 0) {
				foreach ($activities->result() as $row) {
					?><span class="label label-inline label-light-blue label-activity-<?php echo $row->activityID; ?>"
							data-toggle="label-activity-<?php echo $row->activityID; ?>"><?php echo $row->name; ?></span><?php
				}
			}
			?>
			<span class="label label-inline label-light-blue label-activity-other"
				  data-toggle="label-activity-other">Other</span>
		</div>
		<h3>Session Count</h3>
		<p><span class="lesson_count"><?php echo $lesson_count; ?></span> Sessions</p>
		<h3>Session Hours</h3>
		<p><span class="lesson_hours"><?php echo $lesson_hours; ?></span></p>
		<?php
		// confirm timetable
		$data = array(
			'week' => $week,
		...
		$this->load->view('bookings/timetable_confirm', $data);
		?>
	</div>
</div>
<div class='col-sm-12' id="timetable_mobile">
	...
	?>
	<h3>Session Count</h3>
	<p><?php echo $lesson_count; ?> Sessions</p>
	<h3>Session Hours</h3>
	<p><?php echo $lesson_hours; ?></p><?php
	// confirm timetable
	$data = array(
		'week' => $week,
		'year' => $year,
